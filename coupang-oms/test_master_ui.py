...
def main():
    ...
    with sync_playwright() as p:
        ...
        print("\n【1b】線別工具選單（三頁共用）")
        pg.goto(f"{base}/"); pg.wait_for_selector(".lm-split .lm-btn"); pg.wait_for_timeout(400)
        check("首頁是三顆按鈕：寶僑／瑪氏／紙潔", pg.eval_on_selector_all(".lm-split .lm-btn", "els => els.map(e => e.innerText.trim().split(/\\s/)[0])") == ["寶僑", "瑪氏", "紙潔"])
        check("瑪氏、紙潔標「準備中」", pg.eval_on_selector_all(".lm-split .lm-btn.soon", "els => els.length") == 2)
        pg.click('.lm-split[data-line="pg"] .lm-btn'); pg.wait_for_timeout(200)
        check("點寶僑 → 只展開寶僑那欄、有商品主檔自動化", pg.eval_on_selector_all(".lm.open", "els => els.length") == 1 and "商品主檔自動化" in pg.inner_text('.lm-split[data-line="pg"] .lm-pop'))
        pg.click('.lm-split[data-line="mars"] .lm-btn'); pg.wait_for_timeout(200)
        check("點瑪氏 → 寶僑收起、瑪氏展開寫準備中", pg.eval_on_selector_all(".lm.open", "els => els.length") == 1 and "準備中" in pg.inner_text('.lm-split[data-line="mars"] .lm-pop'))
        pg.click('.lm-split[data-line="pg"] .lm-btn'); pg.wait_for_timeout(200)
        pg.click('.lm-split[data-line="pg"] .lm-pop a.lm-item'); pg.wait_for_selector("#dd-line-btn", state="attached"); pg.wait_for_timeout(600)
