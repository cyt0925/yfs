...
def main():
    ...
    with sync_playwright() as p:
        ...
        print("\n【3】各次匯入的變動")
        pg.select_option("#older-imports", index=1); pg.wait_for_timeout(900)
        check("選第 2 次 → 畫面只剩變動的品項、有標籤", pg.eval_on_selector_all("#orders-list .badge", "els => els.map(e => e.innerText).filter(t => t === '新增' || t === '有變').length") > 0)
        check("匯出按鈕改成「匯出這次變動的專案報價檔」", "這次變動" in pg.inner_text("#btn-export-daily"))
        check("月份沒被跳走", pg.input_value("#sel-month") == "2026-09")
        check("對帳卡片有出來且對得起來", "對得起來" in pg.inner_text("#last-import"))
        pg.click("#older-clear"); pg.wait_for_timeout(700)
        check("回全部", "全部訂單" in pg.inner_text("#last-import") and "這次變動" not in pg.inner_text("#btn-export-daily"))
