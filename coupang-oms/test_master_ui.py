...
def main():
    ...
    with sync_playwright() as p:
        ...
        print("\n【3】最近匯入那一行、只看這次變動、匯入紀錄")
        bar = pg.inner_text("#last-import")
        check("上面只有一行：最近匯入＋新增／有變／消失，沒有對帳算式", "最近匯入" in bar and "新增" in bar and "消失" in bar and "＝" not in bar and "第 " not in bar, bar)
        check("全部訂單模式下，最近一次動到的列有色條標籤", pg.eval_on_selector_all("#orders-list tr.chg-new, #orders-list tr.chg-upd, #orders-list tr.chg-gone", "els => els.length") > 0)
        check("行事曆格子右上角有角標，標出這次動到的天", pg.eval_on_selector_all("#cal .day .dc", "els => els.length") > 0)
        pg.click("#btn-last-changes"); pg.wait_for_timeout(900)
        check("只看這次變動 → 行事曆沒動到的天壓淡、動到的天還亮著", pg.eval_on_selector_all("#cal .day.dim", "els => els.length") > 0 and pg.eval_on_selector_all("#cal .day.has:not(.dim)", "els => els.length") > 0)
        check("只看這次變動 → 畫面只剩有標籤的列", pg.eval_on_selector_all("#orders-list table.m tbody tr", "els => els.length") > 0 and pg.eval_on_selector_all("#orders-list table.m tbody tr:not(.chg-new):not(.chg-upd):not(.chg-gone)", "els => els.length") == 0)
        check("匯出按鈕改成「匯出這次變動的專案報價檔」", "這次變動" in pg.inner_text("#btn-export-daily"))
        ...
        check("匯入紀錄抽屜從右邊滑出、一次一列", pg.evaluate("document.getElementById('imp-drawer').classList.contains('open')") and pg.eval_on_selector_all("#imp-drawer .imp-row", "els => els.length") >= 2)
        pg.click("#imp-drawer .imp-row >> nth=1"); pg.wait_for_timeout(900)
        check("點更早那次 → 只看那次的變動，抽屜留著、那列打勾", "只看變動" in pg.inner_text("#last-import") and pg.evaluate("document.getElementById('imp-drawer').classList.contains('open')") and pg.eval_on_selector_all("#imp-drawer .imp-row.on", "els => els.length") == 1)
