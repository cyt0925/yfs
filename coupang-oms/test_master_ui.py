...
def main():
    ...
    with sync_playwright() as p:
        ...
        check("行事曆換成「這批動到的日期」卡片", not pg.evaluate("document.getElementById('cal-batch').classList.contains('hidden')") and pg.eval_on_selector_all("#cal-batch .dchip[data-date]", "els => els.length") >= 1 and pg.evaluate("document.getElementById('cal').classList.contains('hidden')"))
        ...
        pg.click('#cal-quick [data-q="none"]'); pg.wait_for_timeout(700)
        check("全部取消", pg.eval_on_selector_all("#cal .day.on", "els => els.length") == 0)
        pg.click('#cal-quick [data-q="batch"]'); pg.wait_for_timeout(900)
        check("「最近一批動到的天」一鍵選起", pg.eval_on_selector_all("#cal .day.on", "els => els.length") >= 1)
        pg.click('#cal-quick [data-q="none"]'); pg.wait_for_timeout(700)
        ...
        pg.click('#cal-quick [data-q="none"]'); pg.wait_for_timeout(600)
