...
const HOUSE_TEMPLATE = [
  ...
  "5. 檔期、優惠、價格的擺法依【文字排法】；價格永遠是全圖最大的字。",
  "5-1. momo 鐵律：所有 momo 相關元素（圓形 mo 圖示、「momo限定」「momo獨家」字樣、momo logo）一律用 momo 品牌粉紅 (#E5007D 桃紅到 #FF3E9A 粉紅)，白字或粉紅底白字皆可，絕對不可以用黃色、橘色或藍色去畫 momo 元素。mo 點以粉紅圓形 mo 圖示呈現。",
...
const TEXT_LAYOUTS = {
  // auto 由程式輪替 tag → center → side → top（見 pickAutoLayout），刻意不含底部資訊帶
  "tag": "【文字排法：主標左上＋價格牌】主標左上兩行；價格做成右下角斜貼約 12 度的圓形或爆炸形價格牌，牌內「$數字」特大、單位小字；優惠與檔期一行放在價格牌正上方；不要底部資訊帶，文字不可堆在畫面下方三分之一。",
  ...
  "top": "【文字排法：上字下圖】主標與價格全部集中在上半部：主標兩行居中偏左，價格在主標右側或右上角做成斜貼價格牌，優惠與檔期一行小字緊貼主標下方；下半部完全留給產品與主視覺；畫面最下緣不放任何文字。",
  "band": "【文字排法：底部資訊帶（自家慣例，僅在使用者指定時使用）】主標左上兩行；底部一條滿版深色資訊帶，左邊「檔期 momo限定 ▸ 優惠」，右邊特大價格；小字註記在資訊帶最下緣。"
};
const AUTO_LAYOUT_ORDER = ["tag", "center", "side", "top"];
function pickAutoLayout(variantIndex) {
  const key = AUTO_LAYOUT_ORDER[Math.abs(parseInt(variantIndex, 10) || 0) % AUTO_LAYOUT_ORDER.length];
  return TEXT_LAYOUTS[key] + "\n【硬性規定】文字不可以全部堆在畫面下方三分之一；沒有使用者指定就不要做底部滿版資訊帶。";
}
/** 強調色：預設依背景挑對比色，不要每張都黃 */
const ACCENTS = {
  "auto": "【強調色：依背景自動對比】先決定背景主色，再挑強調色：藍色系背景 → 主色白、強調色用桃粉 (#FF3E9A) 或淺青 (#7FE3FF)，不要黃；橘色系背景 → 主色白、強調色深藍或白底橘字；綠色系背景 → 主色白、強調色檸檬綠或白；紅色或深色背景 → 主色白、強調色金或白；淺色背景 → 主色深藍、強調色品牌色。黃色只有使用者明確選「亮黃」時才能用。",
...
function buildChatInstruction(input) {
  ...
  if (input.mode === "background") {
    ...
    if (input.layoutImages && input.layoutImages.length) lines.push("【文字排法：照版型圖】文字位置、資訊帶樣式、強調色都照附上的版型圖，只換內容。");
      ...
      lines.push(TEXT_LAYOUTS[input.textLayout] || pickAutoLayout(input.variantIndex));
      lines.push(ACCENTS[input.accent] || ACCENTS.auto);
    }
    lines.push("");
    if (hasSource) {
      lines.push("【原稿的用法】原稿只是「內容來源」，不是「版型來源」：品牌、產品、所有文字內容與數字以它為準，一字不差地保留，產品照忠實重現。但版面、文字位置、配色、字體處理一律不可沿用原稿，要照上面的【文字排法】與【強調色】重新設計；原稿若是底部資訊帶、藍底黃字，新稿就不可以再是底部資訊帶、藍底黃字。下面若有給欄位，就用欄位的值覆蓋原稿對應的那一項。");
